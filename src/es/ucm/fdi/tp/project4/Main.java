package es.ucm.fdi.tp.project4;
...
import es.ucm.fdi.tp.project4.ataxx.AtaxxFactory;
...
public class Main {
			...
			if (dimensionSpecified() && obstaclesSpecified()) {
				...
				if (correctDimension())
					gameFactory = new AtaxxFactory(dimRows,
							AtaxxFactory.DEFAULT_OBSTACLES);
				else {
					createDefaultAtaxxGame();
				}
				...
				if (correctObstacles())
					gameFactory = new AtaxxFactory(
							AtaxxFactory.DEFAULT_DIM, obstacles);
				else
					createDefaultAtaxxGame();
			} else {
				createDefaultAtaxxGame();
	...
	private static boolean correctObstacles() {
		if (dimRows == null || dimCols == null) {
			dimRows = AtaxxFactory.DEFAULT_DIM;
			dimCols = AtaxxFactory.DEFAULT_DIM;
		}
